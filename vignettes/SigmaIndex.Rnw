...
@
<<si-shc-rqt,fig=TRUE,include=FALSE,echo=FALSE,width=1.3,height=2.5, pdf=FALSE, eps=FALSE, png=TRUE>>=
...
barplot(colMeans(df1), las=2, cex.names=.6, cex.axis=.6, cex.lab=.8, ylab="Relative query time", 
        ylim=c(0,(max(mns)*1.1)), mgp=c(1.5,0.7,0))
@
...
\begin{figure} \centering
	...
	\end{minipage}
	\begin{minipage}[b]{.20\linewidth} \centering
		\includegraphics[width=\linewidth]{SigmaIndex-si-shc-rqt}
		\\(g)
	\end{minipage}
	\begin{minipage}[b]{.20\linewidth} \centering
...
Figures~\ref{fig:si_shc_comparative}a-c shows histograms for all three neighborhood multipliers $nm \in \{2,3,4\}$. Figures~\ref{fig:si_shc_comparative}d-i show the comparison between the \textit{sequential scan} approach and $\Sigma$-index. In Figure~\ref{fig:si_shc_comparative}d we can see the corrected Rand for all four settings. There is no precision degradation when using $\Sigma$-index. In Figure~\ref{fig:si_shc_comparative}e we can see the number of statistical distributions for whose we needed to calculate the Mahalanobis distance. The worst-case scenario when using the $\Sigma$-index requires significantly fewer calculations than the \textit{sequential scan} approach. This pattern is inversely reflected in the computational cost reduction in Figure~\ref{fig:si_shc_comparative}f. We can see that the setting \code{nm = 4} is slightly better than \code{nm = 3}. To compute the relative query time, we captured execution times needed to calculate statistical distances (\ref{eq:statistical_distance}) to classify all $20000$ observations for each setting. Then we compared these query execution times to the longest query execution time, which we expect to be the sequential scan. These query execution relations between settings can be seen in Figure~\ref{fig:si_shc_comparative}g. Absolute processing times in Figure~\ref{fig:si_shc_comparative}h are the execution times needed to process all $20000$ observations for each setting. 

Relative and absolute execution times depend on the execution environment, i.e., operating system, virtualization, compiler, linked libraries, memory management, and similar. In our case, the most important is the reflection of the computational cost reduction pattern to all execution times.

The update time in Figure~\ref{fig:si_shc_comparative}i is the highest in the setting \code{nm = 4} since the $\Sigma$-index DAG is the most interconnected of all settings. 

Figures~\ref{fig:si_shc_comparative}a and \ref{fig:si_shc_comparative}f show a small difference since histograms are calculated in the invocation moment. Applied to SHC, Figure~\ref{fig:si_shc_comparative}f shows the whole data stream evolution, while Figure~\ref{fig:si_shc_comparative}a shows only the final result.
...
@
<<si-shc-rqt2,fig=TRUE,include=FALSE,echo=FALSE,width=1.3,height=2.5,pdf=FALSE,eps=FALSE,png=TRUE>>=
par(mar=c(4, 3, 0.2, 0.2),mgp=c(1.5,0.7,0))
q_max <- max(res1$queryTime,res2$queryTime,res3$queryTime,res4$queryTime)
df1 <- data.frame('SHC(seq.)'=res1$queryTime/q_max,'SHC(nm=2)'=res2$queryTime/q_max,'SHC(nm=3)'=res3$queryTime/q_max,'SHC(nm=4)'=res4$queryTime/q_max,check.names=F)
mns <- colSums(df1)
...
\begin{figure} \centering
		...
		\end{minipage}
		\begin{minipage}[b]{.2\linewidth} \centering
			\includegraphics[width=\linewidth]{SigmaIndex-si-shc-rqt2}
			\\(g)
		\end{minipage}
		\begin{minipage}[b]{.2\linewidth} \centering
